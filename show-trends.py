    ...
    if country:
        try:
            ...
            if trends:
                ...
                # 결과 출력
                result = f"=== {country}의 트렌드 요약 및 설명 ==="
                for trend, issue in trend_issues.items():
                    result += f"\n\n--- {trend} ---\n{issue}"
                return result
            else:
                return f"{country}의 트렌드를 가져오는 데 실패했습니다."
        except ValueError as e:
            return str(e)
    else:
        return "지원되지 않는 국가입니다. 한국, 미국, 일본, 홍콩 중에서 선택해 주세요."
...
if __name__ == "__main__":
    iface = gr.Interface(
        ...
        outputs="text", 
        title="국가별 트렌드 확인", 
        description="한국, 미국, 일본, 홍콩 중 하나를 선택하여 트렌드를 확인하세요.")
